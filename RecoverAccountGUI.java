...
//----------------------------------------
//GUI that has changed password Screens
//and set up security answers Screens
//----------------------------------------
public class RecoverAccountGUI {
	...
	private void initialize() {
		...
		securityQuestionBox.setModel(new DefaultComboBoxModel<String>(
		...
		submitButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				User searchUser = Globals.userDatabase.searchUserID(userIDField
						.getText());
				if (searchUser == null) {
					Object[] options = { "OK" };
					JOptionPane.showOptionDialog(null,
							"Wrong User ID or Answer", "Wrong ID/Answer",
							JOptionPane.DEFAULT_OPTION,
							JOptionPane.WARNING_MESSAGE, null, options,
							options[0]);
				} else {
					returnValue = searchUser.recoverAccount(index,
							attemptedAnswer);
					if (returnValue == -1) {
						Object[] options = { "OK" };
						JOptionPane
								.showOptionDialog(
										null,
										"You have not set up the security features for your account",
					...
					if (returnValue == 1) {
						Object[] options = { "OK" };
						JOptionPane.showOptionDialog(null,
								"Answers do not match.", "Incorrect Answer",
								JOptionPane.DEFAULT_OPTION,
								JOptionPane.WARNING_MESSAGE, null, options,
								options[0]);
					}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ((newPassword.length() < 4) || (newPassword.length() > 11)) {
					...
					if (confirmPassword.equals(newPassword)) {
						...
						Object[] options = { "OK" };
						JOptionPane.showOptionDialog(null, "Password Changed!",
								"Password Changed", JOptionPane.DEFAULT_OPTION,
								JOptionPane.WARNING_MESSAGE, null, options,
								options[0]);
		...
		frame.getContentPane()
				.add(changePasswordLoggedIn, "name_1412434526974");

		changePasswordLoggedIn.setBackground(Color.WHITE);
		frame.getContentPane().add(changePasswordLoggedIn,
				"name_125098458476250");
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if ((answer1Field.getText().toString().length()) <= 0
					...
					Object[] options = { "OK" };
					JOptionPane.showOptionDialog(null,
							"Please Answer all Questions", "Empty Field",
							JOptionPane.DEFAULT_OPTION,
							JOptionPane.WARNING_MESSAGE, null, options,
							options[0]);

				} else {
					Globals.userDatabase.getCurrentUser().setAnswers(
							answer1Field.getText(), answer2Field.getText(),
		...
		JLabel lblWhatIsYour_1 = new JLabel(
				"What is your mother's maiden name?");
